...
import pages.GroupBalanceSheetPage
...
class GroupBalanceSheetControllerSpec extends SpecBase with MockitoSugar {
  ...
  "GroupBalanceSheet Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, groupBalanceSheetRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(GroupBalanceSheetPage, GroupBalanceSheet.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(GroupBalanceSheet.values.head), NormalMode)(request, messages(application)).toString
      }
    }
