...
class GroupBalanceSheetViewSpec extends SpecBase with GuiceOneAppPerSuite {

  val SUT: GroupBalanceSheetView                  = app.injector.instanceOf[GroupBalanceSheetView]
  val formProvider: GroupBalanceSheetFormProvider = app.injector.instanceOf[GroupBalanceSheetFormProvider]
  given request: Request[_]                       = FakeRequest()
  given Messages                                  = app.injector.instanceOf[MessagesApi].preferred(request)

  "GroupBalanceSheetView" - {
    "when using a blank form must generate a view" - {
      ...
      "with the correct heading" in {
        ...
        h1.get(0)
          .text() mustBe "Does the group have a consolidated balance sheet of £2 billion or more in its previous financial year?"
      }
