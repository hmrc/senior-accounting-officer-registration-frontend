...
class GroupAnnualRevenuesViewSpec extends SpecBase with GuiceOneAppPerSuite {

  val SUT: GroupAnnualRevenuesView                  = app.injector.instanceOf[GroupAnnualRevenuesView]
  val formProvider: GroupAnnualRevenuesFormProvider = app.injector.instanceOf[GroupAnnualRevenuesFormProvider]
  given request: Request[_]                         = FakeRequest()
  given Messages                                    = app.injector.instanceOf[MessagesApi].preferred(request)

  "GroupAnnualRevenuesView" - {
    "when using a blank form must generate a view" - {
      ...
      "with the correct heading" in {
        ...
        h1.get(0).text() mustBe "Does the group have a total aggregated annual turnover of £200 million or more in its previous financial year?"
      }
