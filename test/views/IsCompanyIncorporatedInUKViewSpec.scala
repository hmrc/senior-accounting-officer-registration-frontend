...
class IsCompanyIncorporatedInUKViewSpec extends SpecBase with GuiceOneAppPerSuite {

  val SUT: IsIncorporatedUnderUkCompanyActsView = app.injector.instanceOf[IsIncorporatedUnderUkCompanyActsView]
  val formProvider: IsIncorporatedUnderUkCompanyActsFormProvider =
    app.injector.instanceOf[IsIncorporatedUnderUkCompanyActsFormProvider]
  given request: Request[_] = FakeRequest()
  given Messages            = app.injector.instanceOf[MessagesApi].preferred(request)

  "IsCompanyIncorporatedInUKView" - {
    "when using a blank form must generate a view" - {
      ...
      "with the correct heading" in {
        ...
        h1.get(0).text() mustBe "Is the company incorporated in the UK under the Company Acts 2006?"
      }
    ...
    "when using a form with value -> Yes must generate a view" - {
      "with the Yes radio button selected" in {
        val form = formProvider().bind(Map("value" -> "yes"))
        val doc  = Jsoup.parse(SUT(form).toString)
        ...
        radios.get(0).attr("value") mustBe "yes"
        radios.get(0).hasAttr("checked") mustBe true
        radios.get(1).attr("value") mustBe "no"
        radios.get(1).hasAttr("checked") mustBe false
      }
    }

    "when using a form with value -> No must generate a view" - {
      "with the Standalone radio button selected" in {
        val form = formProvider().bind(Map("value" -> "no"))
        val doc  = Jsoup.parse(SUT(form).toString)
