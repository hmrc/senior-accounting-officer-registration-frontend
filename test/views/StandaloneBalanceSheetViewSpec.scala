...
class StandaloneBalanceSheetViewSpec extends SpecBase with GuiceOneAppPerSuite {

  val SUT: StandaloneBalanceSheetView                  = app.injector.instanceOf[StandaloneBalanceSheetView]
  val formProvider: StandaloneBalanceSheetFormProvider = app.injector.instanceOf[StandaloneBalanceSheetFormProvider]
  given request: Request[_]                            = FakeRequest()
  given Messages                                       = app.injector.instanceOf[MessagesApi].preferred(request)

  "StandaloneBalanceSheetView" - {
    "when using a blank form must generate a view" - {
      ...
      "with the correct heading" in {
        ...
        h1.get(0)
          .text() mustBe "Does the company have balance sheet of £2 billion or more in the previous financial year?"
      }
