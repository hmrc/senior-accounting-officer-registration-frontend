...
class ContactRoleViewSpec extends SpecBase with GuiceOneAppPerSuite {
    ...
    ContactType.values.foreach { contactType =>
      s"must generate a view for $contactType contact" - {
        ...
        "with the correct heading" in {
          ...
          h1.get(0).text() mustBe "Enter role"
        }
        ...
        "with the correct hint" in {
          ...
          hint.get(0).text() mustBe "For example, ‘Chief Financial Officer’."
        }
