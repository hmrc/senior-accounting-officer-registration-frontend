...
import views.html.EligibleStandaloneView

import scala.jdk.CollectionConverters.*

class EligibleStandaloneViewSpec extends SpecBase with GuiceOneAppPerSuite {

  val SUT: EligibleStandaloneView = app.injector.instanceOf[EligibleStandaloneView]
  given request: Request[_]       = FakeRequest()
  given Messages                  = app.injector.instanceOf[MessagesApi].preferred(request)

  "EligibleStandaloneView" - {
    "must generate a view" - {
      ...
      "with the correct heading" in {
        ...
        h1.get(0).text() mustBe "You need to register this company to submit a Senior Accounting Officer notification and certificate"
      }
