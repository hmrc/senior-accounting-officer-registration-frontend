    ...
    (identify andThen getData andThen requireData andThen filterCompleted).async { implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(ContactRolePage(contactType), value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(ContactRolePage(contactType), mode, updatedAnswers))
        )
    }
}
